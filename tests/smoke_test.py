...
async def test_db():
    ...
    check("verified default 0", (await db.get_payment(pid))["verified"] == 0)
    await db.set_payment_verified(pid)
    check("verified set", (await db.get_payment(pid))["verified"] == 1)
    await db.set_payment_note(pid, "not paid - revoked")
    check("note set", "revoked" in (await db.get_payment(pid))["note"])
